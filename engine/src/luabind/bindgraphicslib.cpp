...
namespace prev {
	...
	struct LuaBindShaderFunc {
		...
		static int Create(lua_State * L) {
			...
			StrongHandle<VertexShader> vs = VertexShader::Create(vsc);
			StrongHandle<FragmentShader> fs = FragmentShader::Create(fsc);
			StrongHandle<ShaderProgram> sp = ShaderProgram::Create(vs, fs);
	...
	struct LuaBindVertexBufferFunc {
		...
		static int Create(lua_State * L) {
			...
			StrongHandle<VertexBuffer> vb = VertexBuffer::Create(arr, sizeof(float) * size, (unsigned int)stride, BufferUsage::USAGE_STATIC);
	...
	struct LuaBindVertexArrayFunc {
		...
		static int Create(lua_State * L) {
			LUA_CHECK_NUM_PARAMS(1);
			LUA_HANDLED_OBJECT_PARAM(1, VertexShader, vs);

			StrongHandle<VertexArray> va = VertexArray::Create(vs);
	...
	struct LuaBindBufferLayoutFunc {
		...
		static int Create(lua_State * L) {
			...
			StrongHandle<BufferLayout> bl = BufferLayout::Create();
...
#define SET_DATA_TYPE(D) lua_pushstring(L, #D); lua_pushinteger(L, (int)DataType::D); lua_settable(L, -3)
	...
	struct LuaBindFramebufferFunc {
		...
		static int Create(lua_State * L) {
			...
			StrongHandle<Framebuffer> fbo = Framebuffer::Create(ToVec2i(*size), TextureFormat::RGBA, FRAMEBUFFER_NONE);
